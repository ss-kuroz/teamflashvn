...
- **Styles:** Tailwind CSS v4 (browser CDN) with custom `@theme` for the landing page (`index.html`); `css/proposal.css` for Reveal.js presentations. Dark theme with orange/gold accents, glassmorphism cards, and responsive breakpoints.
- **Dependencies (all CDN):** Google Fonts (Inter, Montserrat), FontAwesome 6.4.0, Reveal.js 4.3.1, Tailwind CSS v4 (`@tailwindcss/browser@4`).
...
- **Design tokens:** the site uses a consistent dark theme with `--color-tf-orange: #FF8C00`, `--color-tf-gold: #FFD700`, `--color-tf-green: #4caf50`, `--color-tf-blue: #2196f3`, `--color-tf-red: #ff5252`, and `--color-tf-purple: #b388ff` (defined in Tailwind `@theme`). Reuse these when adding new components.
